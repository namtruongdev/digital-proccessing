...
SimpleImage = function () {
  ...
  var funMap = [
    ...
    function (source) {
      if (source instanceof HTMLImageElement) {
        ...
        return __SimpleImageUtilities.makeHTMLImageFromInput(
          source.files[0],
          this
        );
        ...
        __SimpleImageUtilities.throwError(
          'Unrecognized value used to create a SimpleImage: ' + source
        );
      }
    },
    function (width, height) {
      if (width > 0 && height > 0) {
        ...
        __SimpleImageUtilities.throwError(
          'Unable to create a SimpleImage with a negative width or height [' +
            width +
            'x' +
            height +
            ']'
  ...
  if (
    htmlImage != null &&
    (htmlImage instanceof HTMLCanvasElement || htmlImage.complete)
  ) {
...
SimpleImage.prototype = {
  ...
  getPixel: function (x, y) {
    ...
    __SimpleImageUtilities.rangeCheck(
      x,
      0,
      this.getWidth(),
      'getPixel',
  ...
  setSize: function (width, height) {
    ...
    if (width > 0 && height > 0) {
      ...
      this.imageData = __SimpleImageUtilities.changeSize(
        this.canvas,
        width,
        height
      );
      ...
      __SimpleImageUtilities.throwError(
        'You tried to set the size of a SimpleImage to a negative width or height [' +
          width +
          'x' +
          height +
          ']'
  ...
  drawTo: function (toCanvas) {
    if (this.imageData != null) {
      ...
      toCanvas
        .getContext('2d')
        .drawImage(this.canvas, 0, 0, toCanvas.width, toCanvas.height);
    } else {
      var myself = this;
      setTimeout(function () {
  ...
  __init: function (img) {
    try {
      ...
      this.width =
        'naturalWidth' in img
          ? Math.max(img.naturalWidth, img.width)
          : img.width;
      this.height =
        'naturalHeight' in img
          ? Math.max(img.naturalHeight, img.height)
          : img.height;
      ...
      console.log(err);
      __SimpleImageUtilities.throwError(
        'The name you used to create a SimpleImage was not correct: ' + img.id
      );
    }
  },
  ...
  return {
    ...
    makeHTMLImageFromURL: function (url, simpleImage) {
      ...
      if (url.substr(0, 4) != 'http') {
        ...
        __SimpleImageUtilities.throwError(
          'Unfortunately you cannot create a SimpleImage directly from a URL: ' +
            url
        );
      }
    },
